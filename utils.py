...
def prepare_python_script(file_path: str, prompt_ver: str) -> Union[str, None]:
    """preparing python script from generated answer by ChatGPT"""
    SEPARATOR = "answer:*************************\n"

    # remove the last line
    text = open(file_path, "r",errors="ignore").readlines()
    ...
    tmp_file_path = f"./question_and_answer/{prompt_ver}/tmp/tmp_{uuid.uuid4()}.py"
    if not os.path.exists(f"./question_and_answer/{prompt_ver}/tmp"):
        os.makedirs(f"./question_and_answer/{prompt_ver}/tmp")
...
def run_opentrons_simulate(protocol_file: str, output_filename: str) -> Tuple[str, str, str]:
    ...
    """
    cmd = "opentrons_simulate " + (protocol_file if protocol_file else "ThisFileDoesNotExist.py")
...
def save_prompt_and_answer_with_modelname(prefix, prompt, answer, modelname, temperature, max_tokens, prompt_ver: str = "v1") -> str:
    ...
    filename = Path(f"question_and_answer/{prompt_ver}/{prefix}_{modelname}_token_{max_tokens}_temperature_{temperature}_{current_datetime}_prompt_{prompt_ver}.txt")
...
def init_df_eval(dataset_path: str) -> pd.DataFrame:
    ...
    files = glob.glob(dataset_path + "/v*/*.txt")
